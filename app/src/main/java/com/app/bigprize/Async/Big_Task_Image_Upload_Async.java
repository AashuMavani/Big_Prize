...
public class Big_Task_Image_Upload_Async {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher pocAesCipher;

    public Big_Task_Image_Upload_Async(final Activity activity, String taskId, String taskTitle, String image) {
        this.activity = activity;
        pocAesCipher = new BIG_AES_Cipher();
        try {
            BIG_Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("GT87GL", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("55X0PG", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken));
            jObject.put("TFAZIC", taskId);
            jObject.put("CWH0WJ", taskTitle);
            jObject.put("XKBT5R", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("DHSOWC", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("58NHIT", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("AWWQZH", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("P15KHZ", Build.MODEL);
            jObject.put("E5ZLHJ", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("GZGH5W", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Big_Api_Response> call = apiService.taskImageUpload(requestBodyDetails, body);
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    if (activity instanceof Big_TaskDetailsInfoActivity) {
                        ((Big_TaskDetailsInfoActivity) activity).uploadTaskImageSuccess();
                    }
