...
public class Big_Get_Lucky_Number_Async {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher pocAesCipher;

    public Big_Get_Lucky_Number_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("OIMVXZGP", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("HUFRHV", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("BVGFCC", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("FFEET", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.FCMregId));
            jObject.put("LKHUYG", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("NVGCFCDX", Build.MODEL);
            jObject.put("NVGCFF", Build.VERSION.RELEASE);
            jObject.put("MBHHVFC", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("BCCCC", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("HJVRTDRD", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("BVCXDXS", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("NGFTFR", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            //AppLogger.getInstance().e("Get Lucky Number ENCRYPTED ==>", AES_Cipher.encrypt(POC_Constants.getMKEY(),POC_Constants.getMIV(), jObject.toString()));
            Call<Big_Api_Response> call = apiService.getLuckyNumber(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
...
//            //AppLogger.getInstance().e("RESPONSE", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                ((Big_LuckyNumberDrawActivity) activity).setData(responseModel);
