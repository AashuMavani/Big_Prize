...
public class Big_ValidateUpiAsync {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher pocAesCipher;

    public Big_ValidateUpiAsync(Activity activity, String resultStr) {
        this.activity = activity;
        pocAesCipher = new BIG_AES_Cipher();
        try {
            BIG_Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("SH7BNS7VS", resultStr);
            jObject.put("H6BST6VSG", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("BVDSVCS", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken));
            jObject.put("BCSWSDC", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("VCXSDAA", Build.MODEL);
            jObject.put("ESDXDF", Build.VERSION.RELEASE);
            jObject.put("SWVCFCF", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("CXDZZSZ", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("CWAADCC", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("VCDXSWE", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            BIG_AppLogger.getInstance().e("validateUpiId ORIGINAL ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("validateUpiId ENCRYPTED ==>", pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            Call<Big_Api_Response> call = apiService.validateUpiId(BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
    ...
    private void onPostExecute(Big_Api_Response response) {
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                ((Big_ScanActivity) activity).verifyUpiIdResponse(responseModel);
