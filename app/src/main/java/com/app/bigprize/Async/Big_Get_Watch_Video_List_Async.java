...
public class Big_Get_Watch_Video_List_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;

    public Big_Get_Watch_Video_List_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("JDFDHYFN", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("GFOFFOIHH", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("HVGGU", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("JIGYGI", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.FCMregId));
            jObject.put("GVTCTY", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("NYYUUT", Build.MODEL);
            jObject.put("GVCYRST", Build.VERSION.RELEASE);
            jObject.put("IUHIGNBJ", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("JBVTR", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("JUTCT", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("HVVGTVT", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("VKUV", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            BIG_AppLogger.getInstance().e("Watch Video List DATA ORIGINAL)", "" + jObject.toString());
            BIG_AppLogger.getInstance().e("Watch Video List DATA ENCRYPTED)", "" +pocAesCipher.bytesToHex( pocAesCipher.encrypt(jObject.toString())));
            ...
            Call<Big_Api_Response> call = apiService.getWatchVideoList(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    if (activity instanceof Big_WatchVideoActivity) {
                        ((Big_WatchVideoActivity) activity).setData(responseModel);
                    }
