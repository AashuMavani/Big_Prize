...
public class Big_Save_Daily_Reward_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;


    public Big_Save_Daily_Reward_Async(final Activity ActivityDailyReward) {
        this.activity = ActivityDailyReward;
         pocAesCipher = new BIG_AES_Cipher();

        try {
            BIG_Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("JFDHU", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("MHFOQE", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("PIJCUE", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("MUWIHGR", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("BHJJM", Build.MODEL);
            jObject.put("HVKCXR", Build.VERSION.RELEASE);
            jObject.put("AHSJA", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("RUJFDJ", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("YJMKF", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("JHFKT", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("HUFTFT", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));

            BIG_AppLogger.getInstance().e("POINTS", jObject.toString());
            BIG_AppLogger.getInstance().e("POINTS", pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            ...
            Call<Big_Api_Response> call = apiService.saveDailyReward(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    ((Big_ActivityDailyReward) activity).updateData(responseModel);
