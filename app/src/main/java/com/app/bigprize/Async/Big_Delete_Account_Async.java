...
public class Big_Delete_Account_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;


    public Big_Delete_Account_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("G58W0E", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("ZRPTH1", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("0MG1RQ", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("4GD5FQ", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("O4SUF3", Build.MODEL);
            jObject.put("Z9MYNV", Build.VERSION.RELEASE);
            jObject.put("1DKF0L", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("CV7QZK", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("JBYWIX", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("GVJWWM", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("MBDDY", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            BIG_AppLogger.getInstance().e("Delete Account ORIGINAL ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("Delete Account ENCRYPTED ==>",  pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            Call<Big_Api_Response> call = apiService.deleteAccount(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
    ...
    private void onPostExecute(Big_Api_Response response) {
        try {
            BIG_Common_Utils.dismissProgressLoader();
//            Daily_Reward_Response_Model responseModel = new Gson().fromJson(new String(POC_AES_Cipher.encrypt(POC_Constants.getMKEY(),POC_Constants.getMIV(),jObject.toString())), Daily_Reward_Response_Model.class);
            ...
            BIG_AppLogger.getInstance().e("RESPONSE1111: ", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    BIG_Common_Utils.doLogout(activity);
                } else if (!BIG_Common_Utils.isStringNullOrEmpty(responseModel.getMessage())) {
