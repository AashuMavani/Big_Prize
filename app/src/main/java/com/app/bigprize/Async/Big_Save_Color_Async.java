...
public class Big_Save_Color_Async {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher pocAesCipher;


    public Big_Save_Color_Async(final Activity activity, String point) {
...
//        POC_Common_Utils.setToast(activity, userToken);
        try {
            ...
            jObject.put("GNSIWMBU", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("YFKUF", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("GFCGXGXG", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("FMIHNE", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("MLOMAERU", point);
            jObject.put("NSJJERT", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("GVTTYJT", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("MKQLOST", Build.MODEL);
            jObject.put("YYTFT", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("HBUYGU", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("HBYUT", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            BIG_AppLogger.getInstance().e("Save Color ORIGINAL11 ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("Save Color ORIGINAL11 ==>", pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            ...
            Call<Big_Api_Response> call = apiService.saveColorData(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    //AppLogger.getInstance().e("Save Color RESPONSE ==>", "" + new Gson().toJson(responseModel));
                    ((Big_ColorTapActivity) activity).changeColorValues(responseModel);
