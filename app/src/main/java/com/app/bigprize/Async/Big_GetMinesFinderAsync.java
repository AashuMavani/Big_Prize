...
public class Big_GetMinesFinderAsync {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher cipher;

    public Big_GetMinesFinderAsync(final Activity activity) {
        this.activity = activity;
        cipher = new BIG_AES_Cipher();
        try {
            BIG_Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("K8UFY6DT5", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("BCFDFGC", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken));
            jObject.put("VCDSEGG", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.FCMregId));
            jObject.put("CXDDXDD", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("BCTDEDE", Build.MODEL);
            jObject.put("FXDFVH", Build.VERSION.RELEASE);
            jObject.put("XSWFTC", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("VXDFXV", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("CXSZWCV", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("VCXZSDZ", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("CGXRSGC", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            BIG_AppLogger.getInstance().e("Get Minesweeper ORIGINAL ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("Get Minesweeper ENCRYPTED ==>", cipher.bytesToHex(cipher.encrypt(jObject.toString())));
            Call<Big_Api_Response> call = apiService.getMinesweeper(BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken), String.valueOf(n), cipher.bytesToHex(cipher.encrypt(jObject.toString())));
    ...
    private void onPostExecute(Big_Api_Response response) {
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                ((Big_MinesFinderActivity) activity).setData(responseModel);
