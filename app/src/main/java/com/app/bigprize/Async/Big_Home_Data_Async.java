...
public class Big_Home_Data_Async {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher pocAesCipher;


    public Big_Home_Data_Async
        ...
        try {
            ...
            jObject.put("KSPPOM", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            ...
            Call<Big_Api_Response> call = apiService.getHomeData(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
    ...
    private void onPostExecute(Big_Api_Response response, Activity activity) {
        try {
            BIG_Common_Utils.dismissProgressLoader();
            Big_Response_Model responseModel = new Gson().fromJson(new String(pocAesCipher.decrypt(response.getEncrypt())), Big_Response_Model.class);
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    BIG_SharePrefs.getInstance().putString(BIG_SharePrefs.isShowWhatsAppAuth, responseModel.getIsShowWhatsAppAuth());
                    ...
                    BIG_SharePrefs.getInstance().putString(BIG_SharePrefs.fakeEarningPoint, responseModel.getFakeEarningPoint());
                    BIG_SharePrefs.getInstance().putString(BIG_SharePrefs.HomeData, new Gson().toJson(responseModel));
                    ((Big_MainActivity) activity).setHomeData();
