...
import com.app.bigprize.Activity.Big_AdjoeLeaderboardActivity;
...
public class Big_GetAdjoeLeaderboardDataAsync {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;


    public Big_GetAdjoeLeaderboardDataAsync(final Activity activity) {
        ...
        try {
            ...
            jObject.put("BCDDXDF", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("VCGFXDD", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("CFDXDX", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("JBBYUY", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.FCMregId));
            jObject.put("JHGVTCVCV", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("VCVCXD", Build.MODEL);
            jObject.put("FYUFUY", Build.VERSION.RELEASE);
            jObject.put("GFCXDXD", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("FXGDXDX", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("JGRCTXD", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("GRDEDE", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("BFHCFC", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
...
//            POC_AppLogger.getInstance().e("Get ADJOE LEADERBOARD ENCRYPTED ==>", POC_AES_Cipher.encrypt(POC_Constants.getMKEY(),POC_Constants.getMIV(), jObject.toString()));
            Call<Big_Api_Response> call = apiService.getAdjoeLeaderboardData(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS) || responseModel.getStatus().equals("2")) {
                    ((Big_AdjoeLeaderboardActivity) activity).setData(responseModel);
                } else {
