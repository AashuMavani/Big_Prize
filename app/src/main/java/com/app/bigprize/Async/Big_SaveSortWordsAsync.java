...
public class Big_SaveSortWordsAsync {
    private Activity activity;
    private JSONObject jObject;
    private BIG_AES_Cipher cipher;

    public Big_SaveSortWordsAsync(final Activity activity, String point) {
        this.activity = activity;
        cipher = new BIG_AES_Cipher();
        try {
            BIG_Common_Utils.showProgressLoader(activity);
            jObject = new JSONObject();
            jObject.put("O8GY6GTYX", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("HY7GHDUD", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken));
            jObject.put("DD8GDBD9", point);
            jObject.put("F6DB8DIS", Build.MODEL);
            jObject.put("VXDXVCVC", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("CDXSZC", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("VCXDFXDX", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("VCDXV", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("CDFXDXD", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("VCFDDFGVB", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            BIG_AppLogger.getInstance().e("Save Word Sorting ORIGINAL11 ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("Save Word Sorting ENCRYPTED ==>", cipher.bytesToHex(cipher.encrypt(jObject.toString())));
            ...
            Call<Big_Api_Response> call = apiService.saveWordSorting(BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken), String.valueOf(n), cipher.bytesToHex(cipher.encrypt(jObject.toString())));
...
//            AppLogger.getInstance().e("Save Word Sorting RESPONSE ==>", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS) || responseModel.getStatus().equals("2")) {
                    ((Big_SortWordsActivity) activity).changeCountDataValues(responseModel);
