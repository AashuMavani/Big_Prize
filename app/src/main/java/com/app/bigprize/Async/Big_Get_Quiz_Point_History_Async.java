...
import com.app.bigprize.Activity.Big_QuizGameHistoryActivity;
...
public class Big_Get_Quiz_Point_History_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;
    private String type;

    public Big_Get_Quiz_Point_History_Async(final Activity activity, String type, String pageNo) {
        ...
        try {
            ...
            jObject.put("JRTKDXUTR", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("JGVGH", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("UY8IUYJ", pageNo);
            jObject.put("KJSOLI0AXF", type);

            jObject.put("MVJGVGV", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("KBGHVGH", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("BVFFC", Build.MODEL);
            jObject.put("JHIUHUH", Build.VERSION.RELEASE);
            jObject.put("GFGXV", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("HVGVG", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("VCFGCC", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("JHVGVV", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("KVGVYC", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Big_Api_Response> call = apiService.getQuizHistory(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
        ...
        try {
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    ((Big_QuizGameHistoryActivity) activity).setData(type, responseModel);
