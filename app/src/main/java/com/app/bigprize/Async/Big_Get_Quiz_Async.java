...
public class Big_Get_Quiz_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;


    public Big_Get_Quiz_Async(final Activity activity) {
        ...
        try {
            ...
            jObject.put("MJUTERQXZ", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
                jObject.put("VFCFCF", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("BVFCF", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("GXDXEX", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.FCMregId));
            jObject.put("GFXEGF", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("CFGCF", Build.MODEL);
            jObject.put("HGHCC", Build.VERSION.RELEASE);
            jObject.put("FRXERR", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("HFXDXER", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("VCFCDXC", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("GFXXXEX", BIG_Common_Utils.verifyInstallerId(activity));
            jObject.put("BVFCCX", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            ...
            Call<Big_Api_Response> call = apiService.getQuizData(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
...
//            //AppLogger.getInstance().e("RESPONSE", "" + new Gson().toJson(responseModel));
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                ((Big_QuizGameActivity) activity).setData(responseModel);
