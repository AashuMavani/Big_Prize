...
public class Big_Save_Quiz_Async {
    private Activity activity;
    private JSONObject jObject;
   private BIG_AES_Cipher pocAesCipher;


    public Big_Save_Quiz_Async(final Activity activity, String quizId, String selectedAnswer, String point) {
        ...
        try {
            ...
            jObject.put("INCC8G", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userId));
            jObject.put("CXDXDFX", BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN());
            jObject.put("FYHYDNIFUY", selectedAnswer);
            jObject.put("LOTIPHS8IU", point);
            jObject.put("BNHYSW7GB", quizId);
            jObject.put("GFCCHJH", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AdID));
            jObject.put("GFXDX", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.totalOpen));
            jObject.put("GFDXDX", BIG_SharePrefs.getInstance().getInt(BIG_SharePrefs.todayOpen));
            jObject.put("CVXDFXD", BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.AppVersion));
            jObject.put("CXDFXS", Build.MODEL);
            jObject.put("DXXRRE", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            jObject.put("HVVTT", Settings.Secure.getString(activity.getContentResolver(), Settings.Secure.ANDROID_ID));
            //AppLogger.getInstance().e("Save Quiz ORIGINAL ==>", jObject.toString());
            BIG_AppLogger.getInstance().e("Save Quiz ENCRYPTED ==>",pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
            ...
            Call<Big_Api_Response> call = apiService.saveQuizData(BIG_SharePrefs.getInstance().getBoolean(BIG_SharePrefs.IS_LOGIN) ? BIG_SharePrefs.getInstance().getString(BIG_SharePrefs.userToken) : BIG_Constants.getUSERTOKEN(), String.valueOf(n), pocAesCipher.bytesToHex(pocAesCipher.encrypt(jObject.toString())));
    ...
    private void onPostExecute(Big_Api_Response response) {
        try {
            BIG_Common_Utils.dismissProgressLoader();
            Big_Quiz_Data_Model responseModel = new Gson().fromJson(new String(pocAesCipher.decrypt(response.getEncrypt())), Big_Quiz_Data_Model.class);
            ...
            if (responseModel.getStatus().equals(BIG_Constants.STATUS_LOGOUT)) {
                ...
                if (responseModel.getStatus().equals(BIG_Constants.STATUS_SUCCESS)) {
                    ((Big_QuizGameActivity) activity).updateDataChanges(responseModel);
                } else {
