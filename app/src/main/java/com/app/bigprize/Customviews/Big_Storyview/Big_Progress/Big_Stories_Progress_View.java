package com.app.bigprize.Customviews.Big_Storyview.Big_Progress;
...
public class Big_Stories_Progress_View extends LinearLayout {

    private static final String TAG = Big_Stories_Progress_View.class.getSimpleName();
    ...
    private final List<Big_Pausable_ProgressBar> progressBars = new ArrayList<>();
    ...
    public Big_Stories_Progress_View(Context context) {
        this(context, null);
    }

    public Big_Stories_Progress_View(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public Big_Stories_Progress_View(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public Big_Stories_Progress_View(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void bindViews() {
        ...
        for (int i = 0; i < storiesCount; i++) {
            final Big_Pausable_ProgressBar p = createProgressBar();
            progressBars.add(p);
            addView(p);
            if ((i + 1) < storiesCount) {
                addView(createSpace());
    ...
    private Big_Pausable_ProgressBar createProgressBar() {
        Big_Pausable_ProgressBar p = new Big_Pausable_ProgressBar(getContext());
    ...
    public void skip() {
        ...
        if (current < 0) return;
        Big_Pausable_ProgressBar p = progressBars.get(current);
        isSkipStart = true;
        p.setMax();
    }
    ...
    private Big_Pausable_ProgressBar.Callback callback(final int index) {
        return new Big_Pausable_ProgressBar.Callback() {
            @Override
            ...
            public void onFinishProgress() {
                if (isReverseStart) {
                    ...
                    if (0 <= (current - 1)) {
                        Big_Pausable_ProgressBar p = progressBars.get(current - 1);
     ...
     */
    public void destroy() {
        for (Big_Pausable_ProgressBar p : progressBars) {
            p.clear();
        }
    }
