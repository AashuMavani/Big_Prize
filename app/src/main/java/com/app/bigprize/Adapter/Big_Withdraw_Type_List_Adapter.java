...
public class Big_Withdraw_Type_List_Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public static final int ITEM_AD = 2;
    public static final int ITEM_DATA = 0;
    public List<Big_Withdraw_List> listWithdrawTypes;
    private Context context;
    private ClickListener clickListener;
    private Big_Response_Model responseMain;
    public Big_Withdraw_Type_List_Adapter(List<Big_Withdraw_List> list, Context context, ClickListener clickListener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == ITEM_DATA) {
            return new SavedHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.big_item_withdraw_types_list, parent, false));
        }
        if (viewType == ITEM_AD) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder parentHolder, final int position) {
        try {
            ...
            if (itemViewType == ITEM_DATA) {
                ...
                if (!BIG_Common_Utils.isStringNullOrEmpty(listWithdrawTypes.get(position).getBgImage())) {
                    if (listWithdrawTypes.get(position).getBgImage().endsWith(".json")) {
                       holder.ivBanner.setVisibility(View.GONE);
                        ...
                        BIG_Common_Utils.setLottieAnimation(holder.ltSmallIcon, listWithdrawTypes.get(position).getBgImage());
                ...
                if (!BIG_Common_Utils.isStringNullOrEmpty(listWithdrawTypes.get(position).getIcon())) {
                    if (listWithdrawTypes.get(position).getIcon().endsWith(".json")) {
                        ...
                        BIG_Common_Utils.setLottieAnimation(holder.ltSmallIcon, listWithdrawTypes.get(position).getIcon());
                        holder.ltSmallIcon.setRepeatCount(LottieDrawable.INFINITE);
                    } else {
