...
import com.app.bigprize.Async.Models.Big_MilestoneDataItem;
...
public class Big_ActiveMilestonesAdapter extends RecyclerView.Adapter<Big_ActiveMilestonesAdapter.SavedHolder> {
    public List<Big_MilestoneDataItem> listMilestones;
    private Context context;
    private ClickListener clickListener;
    private Big_Response_Model responseMain;

    public Big_ActiveMilestonesAdapter(List<Big_MilestoneDataItem> list, Context context, ClickListener clickListener) {
    ...
    public SavedHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View view = inflater.inflate(R.layout.big_item_active_milestones, parent, false);
    ...
    public void onBindViewHolder(@NonNull SavedHolder holder, final int position) {
        try {
            if ((position + 1) % 2 == 0 && BIG_Common_Utils.isShowAppLovinNativeAds()) {
                loadAppLovinNativeAds(holder.fl_adplaceholder);
            }
            holder.tvTitle.setText(listMilestones.get(position).getTitle());
            if (!BIG_Common_Utils.isStringNullOrEmpty(listMilestones.get(position).getDescription())) {
            ...
            if (!BIG_Common_Utils.isStringNullOrEmpty(listMilestones.get(position).getButtonName())) {
                holder.tvAction.setVisibility(View.VISIBLE);
                holder.tvAction.setText(listMilestones.get(position).getButtonName());
                Drawable mDrawable = ContextCompat.getDrawable(context, R.drawable.big_ic_btn_gradient_rounded_corner_rect_new);
            ...
            int day = BIG_Common_Utils.getDaysDiff(listMilestones.get(position).getEndDate(), listMilestones.get(position).getTodayDate());
            ...
            if (day > 1) {
                ...
                holder.timer = new CountDownTimer(BIG_Common_Utils.timeDiff(listMilestones.get(position).getTodayDate(), listMilestones.get(position).getEndDate()) * 60000, 1000) {

                    @Override
                    public void onTick(long millisUntilFinished) {
                        holder.tvTimer.setText(BIG_Common_Utils.updateTimeRemainingLuckyNumber(millisUntilFinished));
                    }
