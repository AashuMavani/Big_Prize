...
public class Big_TopOffersListAdapter extends RecyclerView.Adapter<Big_TopOffersListAdapter.ViewHolder> {
    public List<Big_TaskOffer> listTask;
    private Context context;
    private ClickListener clickListener;

    public Big_TopOffersListAdapter(Context context, List<Big_TaskOffer> list, ClickListener fileListClickInterface) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.big_raw_top_offers, parent, false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        try {
            ...
            if (!BIG_Common_Utils.isStringNullOrEmpty(listTask.get(position).getLabel())) {
            ...
            if (listTask.get(position).getPoints().matches("0")) {
                ...
                if (listTask.get(position).getBtnColor() != null && listTask.get(position).getBtnColor().length() > 0) {
                    Drawable mDrawable = ContextCompat.getDrawable(context, R.drawable.big_ic_btn_rounded_corner_pressed);
