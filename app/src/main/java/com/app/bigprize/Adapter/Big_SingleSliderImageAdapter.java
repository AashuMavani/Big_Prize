...
public class Big_SingleSliderImageAdapter extends RecyclerView.Adapter<Big_SingleSliderImageAdapter.ViewHolder> {
    ...
    public Big_SingleSliderImageAdapter(final Activity context, List<Big_Home_Data_Item> data, boolean isGrid, ClickListener clickListener) {
        activity = context;
        this.data = data;
        this.isGrid = isGrid;
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        View v = null;
        if (isGrid) {
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.big_item_home_two_grid, parent, false);
        } else {
            v = LayoutInflater.from(parent.getContext()).inflate(R.layout.big_item_home_single_slider, parent, false);
        }
        ...
        if (isGrid) {
            if (data.get(position).getImages() != null) {
                if (data.get(position).getImages().contains(".json")) {
                    ...
                    BIG_Common_Utils.setLottieAnimation(holder.animation_view, data.get(position).getImages());
            ...
            if (data.get(position).getIsNewLable() != null && data.get(position).getIsNewLable().equals("1")) {
                holder.layoutContent.setBackground(activity.getDrawable(R.drawable.big_border_accent_rectangle));
